...
- **VOIDED на success-сторінці (2026-09-08).** Реалізація 2026-09-06 показувала для VOIDED текст FAILED
  («Банк відхилив платіж») і кнопку повтору оплати, хоч VOIDED — це скасоване замовлення без жодного
  платежу, а `POST /liqpay/checkout` відповідає 400 для скасованого. Тепер VOIDED має власний вигляд:
  нейтральний тон, «Замовлення скасовано», «кошти не списувалися», без дій; §5.x про «без кнопок» для
  VOIDED стало правдою й у тексті (fe `d0dbc52`).
