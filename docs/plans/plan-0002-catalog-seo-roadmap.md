...
**Статус (2026-09-06, вечір):** TD-0008 Approved після рецензії, Plan-0007 написаний, **код
обох PR-ів у `dev`** (be `62b1a59`, fe — той самий день; не запушено, не в проді). Range-фільтрів
свідомо немає: у даних жоден вимір не має числового розкиду (TD-0008 §3.3). Виміри з одним
значенням («Діаметр») тепер не показуються. Лічильник для кольору вже є (`colorOptionsPipeline`),
для решти вимірів — ні; `weight_g` тепер поле варіанта (TD-0006), тож range ваги рахується з
нього, а не з атрибута.
