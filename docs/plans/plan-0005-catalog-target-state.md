...
| 3 | Сторінка товару | 15% → див. додаток | swatch-перемикач над кнопкою купівлі й курована таблиця характеристик — у коді (dev). Відкрито: бренд виробника (з атрибута «Виробник», не з `Vendor`), доставка за вагою, `sku`/`inProductGroupWithID` — усе це блок E → Plan-0006 PR-1, PR-6 |
...
| 5 | Архівний товар | 20% | `by-slug` віддає 404 на ARCHIVED; окремого режиму сторінки немає. Рішення власника 2026-09-06: 200 + `Discontinued` — Plan-0006 задачі 3 (be) і 27 (fe) |
...
| 12 | Нові поля у формах | **0%** | `weight_g` і `google_product_category` не існують у жодному репо. Спроєктовано (TD-0006 §5.2), задачі — Plan-0006 PR-1, PR-5. ⚠️ Артборд показував `google_product_category = 2496` (у Google це «Medical»); правильне значення `499682`, артборд виправлено 2026-09-06 |
| 13 | Статус Google-фіда | **0%** | модуля `feed/` немає, екрана немає, GA4-подій немає. Задачі — Plan-0006 PR-2, PR-4, PR-5 |
...
**Проєктування закрите 2026-09-06 (E1–E3); код не розпочато.** 36 обіцянок
макета, 0 закритих у коді. Два екрани макета (12 і 13) існують лише на папері,
плюс половина сторінки товару. Рецензія TD-0006 знайшла один блокер, який
інакше поїхав би в код: бренд у TD брався з `Vendor.name`, а `Vendor` у базі —
постачальник («Filando», «NicePrice»), не виробник; усі 301 позиція пішли б у
Google з чужим брендом.
...
| E4 | Реалізація за Plan-0006: `weight_g`, `google_product_category` + форми адмінки, ARCHIVED у `by-slug`, модуль `feed/` з генерацією на bootstrap, JSON-LD, GA4, бренд з атрибута, доставка за вагою з чисел NP API, режим «Знято з продажу», екран статусу фіда | — |
...
| G6 | Regression-тест ізоляції категорій для `findCatalogItems` (TD-0005 §7, §8 п.1): дві категорії з однаковим лейблом атрибута, друга не протікає, окремо — під-пайплайн опцій кольору. Взірець — `product-variant-landing-count.int-spec.ts`. fillando-be, S. Власник може відкласти до Фази 4 |
...
| # | Дія | Хто |
|---|---|---|
| H1 | **Ревалідація кешу вітрини в проді.** У `dev` fillando-fe (9c5eb12) є `POST /api/revalidate` з закритим переліком ресурсів і секретом `REVALIDATE_SECRET`; браузерний виклик із форми лендінга скомпільовано лише для розробки, а в проді роут відповідає 503, поки секрету немає. Потрібен server-to-server виклик із бекенду: після запису/публікації/видалення лендінга `LandingService` викликає `${FRONTEND_URL}/api/revalidate` з секретом (fire-and-forget, з логом), ресурс `landings`; той самий секрет в env обох сервісів на Railway. Без цього тексти лендінгів на проді з'являються з затримкою до години або після рестарту | fillando-be (S) + env |
...
блок E — код за Plan-0006 (7 PR-ів), єдиний великий шматок коду в черзі; блок F — одне
рішення власника (лишається в макеті чи ні); блок G — доки після релізу плюс G6 (один тест);
блок H — ревалідація в проді; блок B — тексти й міграції; блок A — сам реліз.
