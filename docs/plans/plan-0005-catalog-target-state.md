...
| A4 | Міграції однією командою: `yarn migrate --dry-run`, потім `yarn migrate` (зупиняється перед утриманими кроками), після живого фронта — `yarn migrate --colors-only` (3j кольори + 3k короткі назви), тоді `yarn migrate:verify` і `POST /api/revalidate {"resource":"landings"}` для sitemap. Відрепетирувано на дампі прода 2026-09-05; **весь ланцюг застосовано на dev 2026-09-07** — сходиться за два проходи, `verify` exit 0. Порахувати легасі `CASH + NOVA_POST` (TD-0009 §8.5): на dev 0 | власник | Блок B |
...
| 4 | §І.8 прогалини тестів: перемикач варіантів, крон фіда, RBAC-спеки шести модулів, тротлер входу. Плюс два дефекти, знайдені самими тестами: ліміт на оптову форму й недосяжна підказка «немає в наявності» | be 3, fe 1 |
...
**Перевірки на 2026-09-09:** BE 56 сюїт / 1284 юніт-тести, 8 сюїт / 80 інтеграційних,
`tsc --noEmit` чисто; FE 51 файл / 567 тестів, прод-збірка проходить. Нових помилок лінту не
додано (у `order.service.ts` навіть на 24 менше). Тестів за сесію додано: BE +335, FE +131.
...
преіснуючі чужі файли. Прогалини в тестах §І.8 закриті хвилею 4, крім RBAC-спек шести модулів без
write-ендпоінтів у вітрині (`payment-providers`, `discount-coupon` за ролями, `cart`, `liqpay`,
`nova-post`, `prom`) — перелічені як відкритий борг у `src/docs/RBAC.md`.
