...
| `PENDING` | «Очікуємо підтвердження оплати…» з опитуванням; після 60 с — нейтральна картка «Статус оплати ще не підтверджено — щойно банк підтвердить, надішлемо лист», без кнопки повтору (друга сесія LiqPay поверх живої — шлях до подвійного списання) |
| `FAILED` / `VOIDED` | **«Оплата не пройшла»**: банк відхилив платіж, кошти не списано, замовлення збережено. Кнопки «Повторити оплату карткою» (повторний `POST /liqpay/checkout` для того ж замовлення, без створення нового) і «Обрати інший спосіб оплати» (поки веде на контакти — зміни способу оплати вже створеного замовлення в інтерфейсі немає, див. Plan-0005 екран 7) |
...
- **Ліміти запитів** (`@nestjs/throttler`, per IP за хвилину): `POST /orders` — 10, `POST /liqpay/checkout` — 10, `POST /discount-coupons/validate` — 20, `GET /orders/lookup/…` — 30, `POST /auth/login` і `/auth/register` — 10, `POST /auth/refresh` — 30, `GET /products/price-sheet` — 20. Перевищення → `429` із заголовком `Retry-After`; на чекауті це тост «Занадто багато спроб. Зачекайте хвилину і спробуйте ще раз.» Глобального ліміту на публічні GET каталогу немає; SSR-запити фронтенда можуть нести `X-Internal-Token` і не тротляться. Деталі — `fillando-be/src/docs/API_AND_SWAGGER.md` §4a.
...
Відповідь — лише `{ order_number, payment_method, payment_status, total_price }`, без
персональних даних. Токен — `HMAC-SHA256(PAYMENT_ENCRYPTION_KEY, "order-lookup:" + orderNumber)`,
