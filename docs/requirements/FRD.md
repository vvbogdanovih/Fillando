...
- `slug`: генерується з назви товару + v_value (напр. `futbolka-bazova-chorna`). Назви товарів після міграції 3k — короткі («Kingroon PLA Silk Rainbow», словник `scripts/fillando_v_2/short-names.js`, TD-0002 §5.2.4); перейменування регенерує slug кожного варіанта без 301 (рішення власника)
